...
int main(){
    ...
    recRoot=(RecNode*)malloc(sizeof(RecNode));//트리를 만든다.
	recRoot->lv=-1;
	recRoot->score=0;
	recRoot->f=field;
...
void play(){
	...
	do{
		...
		command = GetCommand();
		int tmp = ProcessCommand(command);
		if(tmp==QUIT){
			alarm(0);
...
void constructTree(RecNode* root){
	...
	for(i=0;i<CHILDREN_MAX;i++){
		c[i]=(RecNode*)malloc(sizeof(RecNode));//노드 할당
		space+=sizeof(RecNode);
		c[i]->lv=root->lv+1;
		c[i]->f=(char (*)[WIDTH])malloc(sizeof(char)*HEIGHT*WIDTH);//필드 할당
		space+=sizeof(char)*HEIGHT*WIDTH;
...
void destructTree(RecNode* root){
	int i;
	RecNode** c=root->c;
	for(i=0;i<CHILDREN_MAX;i++){
		if(c[i]->lv<VISIBLE_BLOCKS-1){
...
int recommend(RecNode *root){	
	...
	for(r=0;r<NUM_OF_ROTATE;r++){
		...
		rBoundary=WIDTH-rBoundary;
		for(x=lBoundary-2;x<rBoundary;x++,i++){	
			for(h=0;h<HEIGHT;h++){
				for(w=0;w<WIDTH+1;w++){
